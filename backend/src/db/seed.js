...
export async function seedDatabaseIfEmpty() {
  ...
  try {
    ...
    if (!columnNames.includes("amount_paid")) {
      await connection.query("ALTER TABLE bookings ADD COLUMN amount_paid DECIMAL(10,2) DEFAULT 0.00");
      console.log("[MIGRATION] Added amount_paid to bookings table.");
    }
    if (!columnNames.includes("remaining_balance")) {
      await connection.query("ALTER TABLE bookings ADD COLUMN remaining_balance DECIMAL(10,2) DEFAULT NULL");
      console.log("[MIGRATION] Added remaining_balance to bookings table.");
    }

    // 0.1 Create payments table
