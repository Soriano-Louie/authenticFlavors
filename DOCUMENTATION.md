...
- Booking life-cycle state machine: `Pending` → `Reserved` (reservation fee paid) → `Confirmed` (down payment paid) → `Completed` (event date passed). `Cancelled` covers both customer and admin cancellations; the booking is considered forfeited/`Rejected`-equivalent when dismissed by the admin. A fully-paid Final Payment stays on a `Confirmed` booking; the Final Payment is the balance that settles the total price.
- Cancellation charges are **settled in person**: the customer does not upload a receipt, and the admin records the payment as `Paid` with `payment_method = 'Cash'`.
...
| Method | Path                         | Auth   | Description                                                                                            |
| ------ | ---------------------------- | ------ | ------------------------------------------------------------------------------------------------------ |
| `POST` | `/feedback`                  | Bearer | Submit feedback for a completed or cancelled booking (auto-analyzed by AI)    |
...
- [x] Reviews for completed or cancelled bookings (public feedback page too)

### Recent Fixes

- Fixed the **Announcements / Blocked dates** review items and upgraded the admin UI: announcement creation with an expiry no longer crashes (undeclared `publishDateTime`), updates can no longer save an expiry before the publish date, past blocked dates are kept but marked `is_past` ("Past" badge, upcoming-first), and blocking a date that still has active bookings now requires explicit confirmation (`409 DATE_HAS_BOOKINGS`, re-send with `force: true`). The admin **Announcements** page gained an **Expired** filter tab (server-computed `is_expired`), a title/content search bar, and publish-date range filtering — fully responsive. See FIXES.md #49–#53.
- Fixed the remaining **Feedback / Notifications** review items: duplicate feedback now returns `409 ALREADY_SUBMITTED` instead of a 500 (double-click safe); admins are now notified in-app **and** by email on new feedback (`sendNewFeedbackAdminEmail`) and new bookings (`sendNewBookingAdminEmail`); the dormant, unguarded `getFeedbackForBooking` endpoint was removed to close a latent IDOR. See FIXES.md #46–#48.
...
- Feedback is only accepted for bookings whose status is **`Completed` or `Cancelled`** (both submission and the public "what customers say" page). The earlier rule that also allowed past-dated `Confirmed` bookings and limited cancelled reviews to user-cancelled ones (`cancellation_requested_at` set) is removed. The public endpoint `GET /api/feedbacks/public` filters to those statuses and no longer exposes the internal `booking_status` / `cancellation_requested_at` fields. Eligibility is enforced in `createFeedback` (`feedbackController.js`) and in both eligibility filters in `CustomerDashboard.tsx`. Duplicate reviews are prevented by the `uq_feedback_booking` unique key.
