    ...
    <div>
      ...
      <section className="bg-[#2C1810] pt-16 pb-12 px-4">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-[#C8922A] text-xs tracking-widest uppercase font-['Lato'] mb-3">✦ Our Offerings</p>
          <h1 className="font-['Playfair_Display'] text-[#F5F0E8] mb-4" style={{ fontSize: "clamp(1.8rem, 4vw, 3rem)", fontWeight: 600 }}>
            Event & Menu Packages
          </h1>
          <p className="text-[#F5F0E8]/65 font-['Lato'] leading-relaxed max-w-xl mx-auto">
            Discover our thoughtfully crafted event packages — each designed to celebrate your occasion with exquisite food, impeccable service, and unforgettable ambiance.
          </p>
        </div>
      </section>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[#2C1810]/60 text-xs font-['Lato']">Event:</span>
              ...
              {/* Package Type Filter */}
              <div className="flex items-center gap-1.5">
                <span className="text-[#2C1810]/60 text-xs font-['Lato']">Style:</span>
                <div className="flex gap-1.5">
                  {PACKAGE_TYPES.map((t) => (
                    <button
                      key={t}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              <p className="text-[#2C1810]/40 font-['Lato'] text-lg">No packages found for your filters.</p>
              <button
                onClick={() => { setEventFilter("All"); setTypeFilter("All"); setSearch(""); }}
            ...
            <>
              <p className="text-[#2C1810]/50 text-sm font-['Lato'] mb-6">{filtered.length} package{filtered.length !== 1 ? "s" : ""} found</p>
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-7">
                {filtered.map((pkg) => (
                  <div key={pkg.id} className="rounded-2xl overflow-hidden bg-white shadow-md hover:shadow-xl transition-all group">
                    <div
                    ...
                    >
                      ...
                      <div className="absolute top-3 left-3 flex gap-2">
                        <span className="px-2.5 py-1 rounded-full bg-[#C8922A]/90 text-[#F5F0E8] text-[11px] font-['Lato']">{pkg.eventType}</span>
                        <span className="px-2.5 py-1 rounded-full bg-[#2C1810]/70 text-[#F5F0E8] text-[11px] font-['Lato']">{pkg.packageType}</span>
                      </div>

                      <div className="absolute bottom-3 right-3 flex items-center gap-1 bg-[#1A0E08]/70 rounded-full px-2.5 py-1">
                        <Star size={12} className="text-[#C8922A] fill-[#C8922A]" />
                        <span className="text-[#F5F0E8] text-xs font-['Lato']">{pkg.rating}</span>
                      </div>

                      <div className="absolute inset-x-0 bottom-0 p-4">
                        <h3 className="font-['Playfair_Display'] text-[#F5F0E8] text-xl">{pkg.name}</h3>
                      </div>
                    </div>

                    <div className="p-5">
                      <p className="text-[#2C1810]/60 text-sm font-['Lato'] leading-relaxed mb-4 line-clamp-2">{pkg.description}</p>

                      {/* Dishes Preview */}
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {pkg.dishes.map((d) => (
                          <span key={d} className="text-[10px] px-2 py-0.5 rounded-full bg-[#EDE8DF] text-[#C4541A] font-['Lato'] border border-[#C4541A]/15">
                            {d}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-[#C8922A]/10">
                        <div>
                          <p className="text-[#2C1810]/40 text-xs font-['Lato']">Starting from</p>
                          <p className="font-['Playfair_Display'] text-[#2C1810] text-xl">
                            ₱{pkg.pricePerPerson.toLocaleString()}
                            <span className="text-xs text-[#2C1810]/50 font-['Lato']">/person</span>
                          </p>
                        </div>
